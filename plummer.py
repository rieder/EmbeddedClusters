...
class MakePlummerModel(object):
    def __init__(
        ...
        self.mass_cutoff = min(
                mass_cutoff,
                self.calculate_mass_cuttof_from_radius_cutoff(
                    radius_cutoff)
                )
    ...
    def calculate_radius_uniform_distribution(self):
        # r = 1.0 /  numpy.sqrt(
        # numpy.power(numpy.random.uniform(0,self.mass_cutoff,(self.number_of_particles,1)),
        # -2.0/3.0) - 1.0)
        aux = self.random.uniform(0, 1, (self.number_of_particles, 1))
        aux = aux**(2./3.)
        aux = aux/(1.-aux)
        r = numpy.sqrt(aux)
        return r
    ...
    def new_positions_spherical_coordinates(self):
        ...
        theta = numpy.arccos(
                self.random.uniform(
                    -1.0, 1.0, (self.number_of_particles, 1)
                    )
                )
    ...
    def coordinates_from_spherical(self, radius, theta, phi):
        x = radius * numpy.sin(theta) * numpy.cos(phi) * (1-self.xellip)
        y = radius * numpy.sin(theta) * numpy.sin(phi) * (1-self.yellip)
        z = radius * numpy.cos(theta) * (1-self.zellip)
