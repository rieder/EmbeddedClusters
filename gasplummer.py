...
class MakePlummerGasModel(object):
    ...
    def __init__(
        ...
        self.base_sphere = uniform_unit_sphere(target_n, base_grid)
    ...
    def new_model(self):
        ...
        mass = numpy.ones_like(x)/self.actual_n
        internal_energy = (
                self.internal_energy
                / (1+(rtarget/self.rscale)**2)**(1./2)
                )
        r = r.clip(1.e-8, maxr)
        x = rtarget*x/r*(1.-self.xellip)
        y = rtarget*y/r*(1.-self.yellip)
        z = rtarget*z/r*(1.-self.zellip)
    ...
    def result(self):
        ...
        result = datamodel.Particles(self.actual_n)
...
def new_plummer_gas_model(
    ...
    :argument do_scale: scale the result, similar to true nbody units (M=1,
    Q=0.25, U=-0.5)
    """
    uc = MakePlummerGasModel(
            number_of_particles,
            *list_arguments,
            **keyword_arguments)
    return uc.result
