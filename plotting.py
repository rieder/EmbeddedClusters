...
def gas_plot(
    ...
    if p.plot_colormap == "pault_rainbow":
        try:
            from usagi.plotting.colormaps import cm_rainbow
            cmap = cm_rainbow()
        except:
            cmap = "gist_rainbow"
    else:
    ...
    binsq = (
            (
                p.plot_maxx - p.plot_minx
                )
            / p.plot_bins
            * (
                p.plot_maxy - p.plot_miny
                )
            / p.plot_bins
            )
...
def stars_plot(
    ...
    starplot = ax.scatter(
            ...
            marker='o',
            s=0.25 * stars.mass.value_in(units.MSun)**(3./2.),
            color='w',
            lw=0,
            )
    return starplot
